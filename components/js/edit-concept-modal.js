...
function setEditModalCategoryAndEmoji(conceptData) {
  ...
  if (domainField) {
    ...
    setTimeout(() => {
      ...
      if (categoryField) {
        // 카테고리 값이 옵션에 없는 경우 동적으로 추가
        const existingOption = Array.from(categoryField.options).find(
          (option) => option.value === categoryValue
        );
        if (!existingOption && categoryValue) {
          const newOption = document.createElement("option");
          newOption.value = categoryValue;
          newOption.textContent = categoryValue;
          categoryField.appendChild(newOption);
          console.log("📝 카테고리 옵션 동적 추가:", categoryValue);
        }
