# AI 데이터 생성 프롬프트 - Batch 1-16: Education Learning Basic Question
...
- **품사 비율**: pronoun 40%, verb 35%, noun 25%
- **생성 단계**: 1단계 (기초 구축)
...
**생성 요구사항:**
1. concept_id 형식: education_{word}_{meaning} (예: education_ask_question)
...
5. **품사 비율 준수**: pronoun 40%, verb 35%, noun 25% 정확히 반영
6. **CSV 형식**: 쉼표 포함 필드는 쌍따옴표로 감싸기, UTF-8 without BOM 인코딩
7. **빈 필드 처리**: 데이터가 없는 경우 '없음' 대신 빈 칸("") 사용
8. **주제 집중**: 학습 관련 질문, 수업 문의, 이해 확인 등을 포함해주세요

**1단계 (기초 구축) 특화 요구사항:**
- 일상생활 필수 표현 우선 (질문, 문의, 확인, 기본 의문사)
- 기초 어휘 중심 (무엇, 어디, 언제, 어떻게, 왜)
- 단순한 문법 구조 (의문문, 기본 조사, 현재시제)
- 모든 도메인에서 기초 개념 균등 분배

**발음 표기법 (통합_데이터_가이드.md 준수):**
- **Korean**: mu-eot, eo-di (하이픈 구분, 로마자)
- **English**: /wʌt/, /wer/ (IPA 표기)
- **Chinese**: shen me, na li (pinyin, 성조 없음)
- **Japanese**: nani, doko (로마자 헵번식)
- **Spanish**: qué, dón-de (하이픈 구분, 로마자)
