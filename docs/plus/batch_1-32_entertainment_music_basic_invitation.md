# AI 데이터 생성 프롬프트 - Batch 1-32: Entertainment Music Basic Invitation

## 📋 배치 정보
- **배치 번호**: batch_1-32 (50개)
- **도메인**: entertainment (오락)
- **카테고리**: music (음악)
- **난이도**: basic (기초)
- **목적**: invitation (초대)
- **상황**: friendly,concert (친근,콘서트)
- **품사 비율**: verb 35%, noun 35%, preposition 30%
- **생성 단계**: 1단계 (기초 구축)
...
**생성 요구사항:**
1. concept_id 형식: entertainment_{word}_{meaning} (예: entertainment_invite_ask)
...
5. **품사 비율 준수**: verb 35%, noun 35%, preposition 30% 정확히 반영
6. **CSV 형식**: 쉼표 포함 필드는 쌍따옴표로 감싸기, UTF-8 without BOM 인코딩
7. **빈 필드 처리**: 데이터가 없는 경우 '없음' 대신 빈 칸("") 사용
8. **주제 집중**: 콘서트 초대, 음악회 초대, 노래방 초대 등을 포함해주세요

**1단계 (기초 구축) 특화 요구사항:**
- 일상생활 필수 표현 우선 (기본 초대, 음악 용어)
- 기초 어휘 중심 (초대, 같이, 함께, 음악, 노래)
- 단순한 문법 구조 (청유문, 의문문, 기본 동사)
- 모든 도메인에서 기초 개념 균등 분배

**발음 표기법 (통합_데이터_가이드.md 준수):**
- **Korean**: cho-dae-ha-da, gat-i (하이픈 구분, 로마자)
- **English**: /ɪnˈvaɪt/, /æsk/ (IPA 표기)
- **Chinese**: yao qing, yi qi (pinyin, 성조 없음)
- **Japanese**: shotai, issho (로마자 헵번식)
- **Spanish**: in-vi-tar, jun-tos (하이픈 구분, 로마자)
