# AI 데이터 생성 프롬프트 - Batch 1-21: Entertainment Movie Basic Opinion

## 📋 배치 정보
- **배치 번호**: batch_1-21 (50개)
- **도메인**: entertainment (엔터테인먼트)
- **카테고리**: movie (영화)
- **난이도**: basic (기초)
- **목적**: opinion (의견 표현)
- **상황**: casual,travel (캐주얼,여행)
- **품사 비율**: adjective 40%, verb 30%, noun 30%
- **생성 단계**: 1단계 (기초 구축)
...
**생성 요구사항:**
1. concept_id 형식: entertainment_{word}_{meaning} (예: entertainment_good_quality)
...
5. **품사 비율 준수**: adjective 40%, verb 30%, noun 30% 정확히 반영
6. **CSV 형식**: 쉼표 포함 필드는 쌍따옴표로 감싸기, UTF-8 without BOM 인코딩
7. **빈 필드 처리**: 데이터가 없는 경우 '없음' 대신 빈 칸("") 사용
8. **주제 집중**: 영화 평가, 감상 의견, 장르 선호도 등을 포함해주세요

**1단계 (기초 구축) 특화 요구사항:**
- 일상생활 필수 표현 우선 (기본 의견 표현, 영화 용어)
- 기초 어휘 중심 (좋다, 재미있다, 영화, 배우, 스토리)
- 단순한 문법 구조 (현재시제, be동사, 기본 조사)
- 모든 도메인에서 기초 개념 균등 분배

**발음 표기법 (통합_데이터_가이드.md 준수):**
- **Korean**: jo-ta, jae-mi-it-da (하이픈 구분, 로마자)
- **English**: /ɡʊd/, /ˈɪntrɪstɪŋ/ (IPA 표기)
- **Chinese**: hao, you qu (pinyin, 성조 없음)
- **Japanese**: ii, omoshiroi (로마자 헵번식)
- **Spanish**: bue-no, in-te-re-san-te (하이픈 구분, 로마자)
