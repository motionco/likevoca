# AI 데이터 생성 프롬프트 - Batch 1-18: Nature Animal Basic Emotion
...
- **품사 비율**: noun 45%, adjective 35%, interjection 20%
- **생성 단계**: 1단계 (기초 구축)
...
**생성 요구사항:**
1. concept_id 형식: nature_{word}_{meaning} (예: nature_dog_pet)
...
5. **품사 비율 준수**: noun 45%, adjective 35%, interjection 20% 정확히 반영
6. **CSV 형식**: 쉼표 포함 필드는 쌍따옴표로 감싸기, UTF-8 without BOM 인코딩
7. **빈 필드 처리**: 데이터가 없는 경우 '없음' 대신 빈 칸("") 사용
8. **주제 집중**: 동물에 대한 감정, 애완동물 사랑, 자연에 대한 감탄 등을 포함해주세요

**1단계 (기초 구축) 특화 요구사항:**
- 일상생활 필수 표현 우선 (기본 동물 이름, 감정 표현)
- 기초 어휘 중심 (개, 고양이, 예쁘다, 귀엽다, 좋아하다)
- 단순한 문법 구조 (현재시제, be동사, 기본 조사)
- 모든 도메인에서 기초 개념 균등 분배

**발음 표기법 (통합_데이터_가이드.md 준수):**
- **Korean**: gae, go-yang-i (하이픈 구분, 로마자)
- **English**: /dɒɡ/, /kæt/ (IPA 표기)
- **Chinese**: gou, mao (pinyin, 성조 없음)
- **Japanese**: inu, neko (로마자 헵번식)
- **Spanish**: pe-rro, ga-to (하이픈 구분, 로마자)
