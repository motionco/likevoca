...
async function waitForFirebase() {
    ...
    while (attempts < 50) { // Wait up to 5 seconds
        if (window.firebaseInit && window.firebaseInit.db && window.firebaseInit.collection && 
            window.firebaseInit.query && window.firebaseInit.where && 
            window.firebaseInit.getDocs && window.firebaseInit.doc && 
            window.firebaseInit.getDoc && window.firebaseInit.setDoc && 
            window.firebaseInit.updateDoc && window.firebaseInit.addDoc) {
    ...
    console.log('Available window properties:', Object.keys(window).filter(k => k.includes('firebase') || k.includes('db') || k.includes('auth')));
...
async function loadProgressData() {
    ...
    try {
        ...
        if (!window.firebaseInit || !window.firebaseInit.query || !window.firebaseInit.collection || 
            !window.firebaseInit.getDocs || !window.firebaseInit.where) {
        ...
        const { query, where, collection, getDocs, doc, getDoc, setDoc, updateDoc, addDoc, db } = window.firebaseInit;
        ...
        allLearningRecords = learningSnapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }));
        ...
        allGameRecords = gameSnapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }));
        ...
        allQuizRecords = quizSnapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }));
...
async function initializeTargetLanguageStructure() {
    ...
    try {
        ...
        allLearningRecords.forEach(record => {
            ...
            if (record.language_pair?.target) activeLanguages.add(record.language_pair.target);
            if (record.metadata?.targetLanguage) activeLanguages.add(record.metadata.targetLanguage);
        });
...
function calculateCompleteStatsForTargetLanguage(targetLanguage) {
    const learningRecords = allLearningRecords.filter(record => 
        ...
        (record.languagePair && record.languagePair.includes && record.languagePair.includes(targetLanguage)) ||
        (record.language_pair && record.language_pair.target === targetLanguage) ||
        (record.language_pair && record.language_pair.includes && record.language_pair.includes(targetLanguage)) ||
        // metadata에서도 확인
    ...
    const masteredConcepts = Array.from(conceptsMap.values())
        .filter(r => {
            ...
            const accuracy = r.accuracy || r.success_rate || r.performance || 
                           r.score || r.session_quality || r.completion_rate ||
                           r.average_performance || r.learning_efficiency || 
        ...
        .map(r => {
            ...
            return {
                conceptId: conceptId,
                accuracy: r.accuracy || r.success_rate || r.performance || 
                         r.score || r.session_quality || r.completion_rate ||
                         r.average_performance || r.learning_efficiency || 
                         r.comprehension_rate || 
                         (r.concepts_studied ? Math.min(85, r.concepts_studied * 20) : 0),
    ...
    const learning_stats = {
        ...
        total_time: learningRecords.reduce((sum, r) => sum + (r.studyTime || r.session_duration || 0), 0),
        concepts_learned: conceptsMap.size,
        avg_accuracy: learningRecords.length > 0 ? 
            learningRecords.reduce((sum, r) => {
                return sum + (r.accuracy || r.success_rate || r.performance || 
                            r.score || r.session_quality || r.completion_rate ||
                            r.average_performance || r.learning_efficiency || 
                            r.comprehension_rate || 
                            (r.concepts_studied ? Math.min(85, r.concepts_studied * 20) : 0));
            }, 0) / learningRecords.length : 0,
        avg_quality: learningRecords.length > 0 ?
            learningRecords.reduce((sum, r) => sum + (r.session_quality || r.quality || 
                                                    (r.concepts_studied ? Math.min(90, r.concepts_studied * 22) : 0)), 0) / learningRecords.length : 0,
        last_session_date: learningRecords.length > 0 ? 
            learningRecords.sort((a, b) => new Date(b.timestamp) - new Date(a.timestamp))[0].timestamp : null
    };

    // Game stats
    const game_stats = {
        total_games: gameRecords.length,
        total_time: gameRecords.reduce((sum, r) => sum + (r.timeTaken || r.timeSpent || r.time_spent || r.duration || 0), 0),
        avg_score: gameRecords.length > 0 ? 
            gameRecords.reduce((sum, r) => sum + (r.score || r.correct_answers || r.points || 0), 0) / gameRecords.length : 0,
        best_score: gameRecords.length > 0 ? Math.max(...gameRecords.map(r => r.score || r.correct_answers || r.points || 0)) : 0,
        last_game_date: gameRecords.length > 0 ?
            gameRecords.sort((a, b) => new Date(b.timestamp) - new Date(a.timestamp))[0].timestamp : null
    };

    // Quiz stats
    const quiz_stats = {
        total_quizzes: quizRecords.length,
        total_questions: quizRecords.reduce((sum, r) => sum + (r.totalQuestions || r.total_questions || 0), 0),
        correct_answers: quizRecords.reduce((sum, r) => sum + (r.correctAnswers || r.correct_answers || 0), 0),
        avg_accuracy: 0,
        best_accuracy: 0,
        total_time: quizRecords.reduce((sum, r) => sum + (r.timeTaken || r.time_taken || r.duration || 0), 0),
        avg_time_per_quiz: quizRecords.length > 0 ? 
            quizRecords.reduce((sum, r) => sum + (r.timeTaken || r.time_taken || r.duration || 0), 0) / quizRecords.length : 0,
        best_score: quizRecords.length > 0 ? Math.max(...quizRecords.map(r => r.score || r.accuracy || 0)) : 0,
        recent_scores: quizRecords.slice(0, 10).map(r => r.score || r.accuracy || 0)
    };

    // Calculate quiz accuracy
    if (quiz_stats.total_questions > 0) {
        quiz_stats.avg_accuracy = (quiz_stats.correct_answers / quiz_stats.total_questions) * 100;
    }
...
function calculateStatsForTargetLanguage(targetLanguage) {
    ...
    return {
        learning: {
            ...
            mastered_concepts: learningRecords.filter(r => (r.accuracy || 0) >= 80).length,
            average_accuracy: learningRecords.length > 0 ? 
                learningRecords.reduce((sum, r) => sum + (r.accuracy || 0), 0) / learningRecords.length : 0,
            total_study_time: learningRecords.reduce((sum, r) => sum + (r.studyTime || 0), 0)
        },
        games: {
            total_games: gameRecords.length,
            average_score: gameRecords.length > 0 ? 
                gameRecords.reduce((sum, r) => sum + (r.score || 0), 0) / gameRecords.length : 0,
            best_score: gameRecords.length > 0 ? Math.max(...gameRecords.map(r => r.score || 0)) : 0,
            total_time_played: gameRecords.reduce((sum, r) => sum + (r.timeTaken || 0), 0)
        },
        quizzes: {
            total_quizzes: quizRecords.length,
            average_score: quizRecords.length > 0 ? 
                quizRecords.reduce((sum, r) => sum + (r.score || 0), 0) / quizRecords.length : 0,
            best_score: quizRecords.length > 0 ? Math.max(...quizRecords.map(r => r.score || 0)) : 0,
            total_correct: quizRecords.reduce((sum, r) => sum + (r.correctAnswers || 0), 0),
            total_questions: quizRecords.reduce((sum, r) => sum + (r.totalQuestions || 0), 0)
        }
    };
}
...
function calculateTargetLanguageStats(targetLanguage) {
    ...
    if (savedStats) {
        if (savedStats.learning_stats) {
            ...
            stats.lastStudyDate = savedStats.learning_stats.last_session_date ? 
                (savedStats.learning_stats.last_session_date.toDate ? 
                 savedStats.learning_stats.last_session_date.toDate() : 
                 new Date(savedStats.learning_stats.last_session_date)) : null;
        }
        ...
        if (savedStats.quiz_stats) {
            ...
            stats.quizStats.correctAnswers = savedStats.quiz_stats.correct_answers || 0;
            stats.quizStats.totalQuestions = savedStats.quiz_stats.total_questions || 0;
        }
    ...
    const learningRecords = allLearningRecords.filter(record => {
        ...
        const hasLanguagePairIncludes = record.languagePair && record.languagePair.includes && record.languagePair.includes(targetLanguage);
        const hasLanguage_pairTarget = record.language_pair && record.language_pair.target === targetLanguage;
        const hasLanguage_pairIncludes = record.language_pair && record.language_pair.includes && record.language_pair.includes(targetLanguage);
        const hasMetadataTargetLanguage = record.metadata && record.metadata.targetLanguage === targetLanguage;
        const hasTarget_language = record.target_language === targetLanguage;
        
        // activity_type이 vocabulary이고 english 언어인 경우를 기본으로 포함
        const isVocabularyActivity = record.activity_type === 'vocabulary' && targetLanguage === 'english';
    ...
    console.log(`📚 Learning filtering for ${targetLanguage}:`, {
        ...
        allRecordKeys: allLearningRecords[0] ? Object.keys(allLearningRecords[0]) : []
    });
    
    console.log('📚 Full learning record:', allLearningRecords[0]);

    const gameRecords = allGameRecords.filter(record => {
        return record.targetLanguage === targetLanguage ||
               (record.languagePair && record.languagePair.includes(targetLanguage)) ||
               (record.language_pair && record.language_pair.target === targetLanguage) ||
               (record.language_pair && record.language_pair.includes && record.language_pair.includes(targetLanguage)) ||
               (record.metadata && record.metadata.targetLanguage === targetLanguage);
    ...
    const quizRecords = allQuizRecords.filter(record => {
        ...
        const hasTargetLanguage = record.targetLanguage === targetLanguage;
        const hasLanguagePairIncludes = record.languagePair && record.languagePair.includes(targetLanguage);
        const hasLanguage_pairTarget = record.language_pair && record.language_pair.target === targetLanguage;
        const hasMetadataTargetLanguage = record.metadata && record.metadata.targetLanguage === targetLanguage;
        const hasTarget_language = record.target_language === targetLanguage;
        
        // 퀴즈 타입이 translation이고 english 언어인 경우를 기본으로 포함
        const isTranslationQuiz = record.quiz_type === 'translation' && targetLanguage === 'english';
    ...
    learningRecords.forEach(record => {
        ...
        conceptIds.forEach(conceptId => {
            if (conceptId) {
                const accuracy = record.accuracy || record.success_rate || record.session_quality || 40;
    ...
    quizRecords.forEach(record => {
        ...
        if (Array.isArray(record.concept_id)) {
            ...
            conceptIds = Array.isArray(record.quizData.concepts) ? record.quizData.concepts : [record.quizData.concepts];
        }

        conceptIds.forEach(conceptId => {
            if (conceptId) {
                const accuracy = record.correctAnswers && record.totalQuestions ? 
    ...
    gameRecords.forEach(record => {
        ...
        if (Array.isArray(record.concept_id)) {
            ...
            conceptIds = Array.isArray(record.gameData.concepts) ? record.gameData.concepts : [record.gameData.concepts];
        }

        conceptIds.forEach(conceptId => {
            if (conceptId) {
                const accuracy = record.score ? Math.min(95, Math.max(50, record.score / 3)) : 70;
                allConceptsMap.set(conceptId, {
                    ...record,
                    conceptId,
                    activity_type: 'game',
    ...
    stats.masteredConcepts = Array.from(allConceptsMap.values()).filter(concept => 
        concept.calculated_accuracy >= 80
    ).length;
    ...
    if (learningRecords.length > 0) {
        ...
        const sortedDates = [...new Set(allActivities.map(r => {
    ...
    if (gameRecords.length > 0) {
        ...
        const times = gameRecords.map(r => {
            const time = r.timeTaken || r.timeSpent || r.time_spent || r.duration || 0;
        ...
        stats.gameStats.averageScore = scores.reduce((a, b) => a + b, 0) / scores.length;
    ...
    if (quizRecords.length > 0) {
        ...
        stats.quizStats.averageScore = scores.reduce((a, b) => a + b, 0) / scores.length;
        ...
        console.log('🎯 Quiz stats calculated:', {
            ...
            calculatedAccuracy: totalQuestions > 0 ? (totalCorrect / totalQuestions * 100) : 0
        });
    }
...
function updateSummaryCards(stats) {
    ...
    if (studyStreakEl) {
        studyStreakEl.textContent = `${stats.studyStreak}일`;
    }
    ...
    if (totalQuizzesEl) {
        totalQuizzesEl.textContent = `${stats.quizStats.totalQuizzes}회`;
    }
    ...
    if (totalGamesEl) {
        totalGamesEl.textContent = `${stats.gameStats.totalGames}회`;
    }

    // Average game score (게임 성취도)
    const avgGameScoreEl = document.getElementById('avg-game-score');
    if (avgGameScoreEl) {
        avgGameScoreEl.textContent = `${Math.round(stats.gameStats.averageScore)}점`;
    }

    // Learning sessions (학습 성취도) - 실제 학습 기록 수 사용
    const totalLearningSessionsEl = document.getElementById('total-learning-sessions');
    if (totalLearningSessionsEl) {
        ...
        totalLearningSessionsEl.textContent = `${learningRecords.length}회`;
        console.log(`🎓 수정된 학습 세션 수: ${learningRecords.length}회 (전체 학습 기록: ${allLearningRecords.length}개, 개념 수: ${stats.totalConcepts}개)`);
    }
    ...
    if (totalStudyTimeEl) {
        ...
        if (hours > 0) {
            ...
            totalStudyTimeEl.textContent = `${minutes}분`;
        }
    }
    ...
    if (completionRateEl) {
        const rate = stats.totalConcepts > 0 ? 
            Math.round((stats.masteredConcepts / stats.totalConcepts) * 100) : 0;
...
function updateCharts(stats) {
    // This function can be expanded to include chart libraries like Chart.js
...
function showQuizAccuracyDetails() {
    ...
    if (modalBody) {
        modalBody.innerHTML = `
            <div class="space-y-4">
                <h3 class="text-lg font-semibold text-gray-800">퀴즈 정확도 상세 정보</h3>
                <div class="grid grid-cols-2 gap-4">
                    <div class="bg-purple-50 p-4 rounded-lg">
                        <p class="text-sm text-gray-600">총 퀴즈 수</p>
                        <p class="text-xl font-bold text-purple-600">${stats.quizStats.totalQuizzes}회</p>
                    </div>
                    <div class="bg-blue-50 p-4 rounded-lg">
                        <p class="text-sm text-gray-600">총 문제 수</p>
                        <p class="text-xl font-bold text-blue-600">${stats.quizStats.totalQuestions}문제</p>
                    </div>
                    <div class="bg-green-50 p-4 rounded-lg">
                        <p class="text-sm text-gray-600">정답 수</p>
                        <p class="text-xl font-bold text-green-600">${stats.quizStats.correctAnswers}개</p>
                    </div>
                    <div class="bg-orange-50 p-4 rounded-lg">
                        <p class="text-sm text-gray-600">정확도</p>
                        <p class="text-xl font-bold text-orange-600">${Math.round(accuracy)}%</p>
                    </div>
                </div>
                <div class="bg-gray-50 p-4 rounded-lg">
                    <p class="text-sm text-gray-600 mb-2">평균 점수</p>
                    <p class="text-lg font-semibold">${Math.round(stats.quizStats.averageScore)}점</p>
                    <p class="text-sm text-gray-500">최고 점수: ${stats.quizStats.bestScore}점</p>
                </div>
            </div>
        `;
...
async function showTotalWordsDetails() {
    ...
    if (modalBody) {
        ...
        for (const [conceptId, conceptData] of conceptsMap) {
            ...
            const averageAccuracy = activityCount > 0 ? Math.round(totalAccuracy / activityCount) : 0;
        ...
        const conceptsList = detailedConceptsList.map(concept => {
            ...
            const progressBarColor = concept.averageAccuracy >= 80 ? 'bg-green-500' : 
                                   concept.averageAccuracy >= 60 ? 'bg-yellow-500' : 'bg-red-500';
            
            return `
                <div class="border rounded-lg p-4 ${concept.isMastered ? 'bg-green-50 border-green-200' : 'bg-white border-gray-200'} shadow-sm">
                    <div class="flex items-center justify-between mb-3">
                        ...
                        <div class="text-right">
                            <span class="px-2 py-1 text-xs font-medium rounded-full ${concept.isMastered ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800'}">
                                ${concept.isMastered ? '🏆 마스터' : '📚 학습중'}
                            </span>
                        </div>
                    </div>
                    ...
                    <div class="mb-3">
                        <div class="flex justify-between items-center mb-1">
                            <span class="text-xs text-gray-600">🎯 마스터 진행률</span>
                            <span class="text-xs font-medium text-gray-900">${concept.averageAccuracy}%</span>
                        </div>
                    ...
                    <div class="flex items-center justify-center text-sm text-gray-600 space-x-2">
                        <span class="flex items-center">
                            ...
                            <span class="font-bold text-orange-600">${concept.totalActivities}</span>
                        </span>
                        <span class="text-gray-400">|</span>
                        <span class="flex items-center">
                            <span class="text-green-500 mr-1">📚</span>
                            <span class="font-bold text-green-600">${concept.learningCount}</span>
                        </span>
                        <span class="text-gray-400">|</span>
                        <span class="flex items-center">
                            <span class="text-purple-500 mr-1">🎮</span>
                            <span class="font-bold text-purple-600">${concept.gameCount}</span>
                        </span>
                        <span class="text-gray-400">|</span>
                        <span class="flex items-center">
            ...
            <div class="space-y-6">
                ...
                <div>
                    <div class="flex items-center justify-between mb-4">
                        <div class="flex items-center space-x-3">
                            <h3 class="text-lg font-semibold text-gray-800">📚 학습한 단어 ${stats.totalConcepts}개</h3>
                        </div>
                        <!-- 이모지 설명 -->
                        <div class="text-xs text-gray-500 text-right">
                            <div>📚 학습 | 🎮 게임 | 🎯 퀴즈 | 📊 전체활동</div>
                        </div>
                    </div>
                    <div class="space-y-3">
                        ${conceptsList.length > 0 ? conceptsList : '<div class="text-center py-8"><p class="text-gray-500">📭 학습한 개념이 없습니다.</p><p class="text-sm text-gray-400 mt-1">학습을 시작하여 개념을 추가해보세요!</p></div>'}
                    </div>
                </div>
                ...
                <div class="bg-gradient-to-r from-blue-50 to-green-50 p-6 rounded-lg border">
                    <div class="flex justify-between items-center mb-3">
                        ...
                        <span class="text-sm text-gray-600">${stats.masteredConcepts}/${stats.totalConcepts}</span>
                    </div>
                    <div class="w-full bg-gray-200 rounded-full h-4">
                        <div class="bg-gradient-to-r from-blue-500 to-green-500 h-4 rounded-full transition-all duration-500" style="width: ${stats.totalConcepts > 0 ? (stats.masteredConcepts / stats.totalConcepts * 100) : 0}%"></div>
                    </div>
                    <div class="flex justify-between items-center mt-2">
                        <p class="text-sm text-gray-600">${stats.totalConcepts > 0 ? Math.round((stats.masteredConcepts / stats.totalConcepts) * 100) : 0}% 완료</p>
...
function showMasteredWordsDetails() {
    ...
    if (modalBody) {
        ...
        const masteredConcepts = Array.from(conceptsMap.values()).filter(record => {
            // 더 포괄적인 정확도 계산으로 마스터 여부 판단
            const accuracy = record.accuracy || record.success_rate || record.performance || 
                           record.score || record.session_quality || record.completion_rate ||
                           record.average_performance || record.learning_efficiency || 
                           record.comprehension_rate || 
                           (record.concepts_studied ? Math.min(85, record.concepts_studied * 20) : 0);
            return accuracy >= 70; // 더 관대한 기준
        });
        ...
        const masteredList = masteredConcepts.map(record => {
            ...
            const studyTime = record.studyTime || record.session_duration || 0;
                ...
                <div class="flex items-center justify-between p-3 bg-green-50 border border-green-200 rounded-lg">
                    <div class="flex items-center space-x-3">
                        ...
                        <div>
                            ...
                            <p class="text-sm text-gray-500">정확도: ${accuracy}% | 학습시간: ${studyTime}분</p>
                        </div>
                    </div>
                    <div class="text-right">
            ...
            <div class="space-y-6">
                ...
                <div class="bg-green-50 p-4 rounded-lg">
                    <div class="flex items-center justify-between">
                        <div>
                            <h3 class="text-lg font-semibold text-green-800">🎉 마스터 완료!</h3>
                            <p class="text-sm text-green-600">정확도 80% 이상 달성한 개념들</p>
                        </div>
                        <div class="text-right">
                            <div class="text-2xl font-bold text-green-600">${stats.masteredConcepts}</div>
                            <div class="text-sm text-green-600">/ ${stats.totalConcepts}개</div>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-gray-50 p-4 rounded-lg">
                    <div class="flex justify-between items-center mb-2">
                        <span class="font-medium">전체 마스터 진행률</span>
                        <span class="text-sm text-gray-600">${stats.totalConcepts > 0 ? Math.round((stats.masteredConcepts / stats.totalConcepts) * 100) : 0}%</span>
                    </div>
                    <div class="w-full bg-gray-200 rounded-full h-3">
                        <div class="bg-green-500 h-3 rounded-full transition-all duration-300" style="width: ${stats.totalConcepts > 0 ? (stats.masteredConcepts / stats.totalConcepts * 100) : 0}%"></div>
                    </div>
                    <div class="mt-3 text-center">
                        ${stats.masteredConcepts > 0 ? 
                            `<p class="text-sm text-green-600">🌟 훌륭합니다! ${stats.masteredConcepts}개 개념을 마스터했어요!</p>` :
                            `<p class="text-sm text-gray-600">💪 첫 번째 마스터를 향해 도전해보세요!</p>`
                        }
                    </div>
                </div>
            </div>
        ...
        const modalTitle = document.querySelector('#totalWordsModal h2');
        if (modalTitle) {
            modalTitle.textContent = '✅ 마스터한 단어';
        }
            ...
            <div class="space-y-4">
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div class="bg-orange-50 p-4 rounded-lg">
                        ...
                        <p class="text-xl font-bold text-orange-600">${stats.studyStreak}일</p>
                    </div>
                    <div class="bg-blue-50 p-4 rounded-lg">
                        <p class="text-sm text-gray-600">최근 학습일</p>
                ...
                <div class="bg-gray-50 p-4 rounded-lg">
                    ...
                    <div class="space-y-2">
                        <div class="flex justify-between">
                            <span class="text-sm">총 학습 세션:</span>
                            <span class="font-medium">${stats.totalConcepts}회</span>
                        </div>
                        <div class="flex justify-between">
                            <span class="text-sm">총 학습 시간:</span>
                            <span class="font-medium">${Math.floor(stats.totalStudyTime / 60)}시간 ${stats.totalStudyTime % 60}분</span>
                        </div>
                        <div class="flex justify-between">
                            <span class="text-sm">평균 정확도:</span>
                            <span class="font-medium">${Math.round(stats.averageAccuracy)}%</span>
                        </div>
                    </div>
                </div>
                <div class="bg-orange-50 p-4 rounded-lg">
                    <p class="text-sm text-gray-600">🔥 연속 학습을 유지하려면 매일 꾸준히 학습하세요!</p>
                </div>
            </div>
        `;
            ...
            return `
                <div class="flex items-center justify-between p-3 bg-purple-50 border border-purple-200 rounded-lg">
                    <div class="flex items-center space-x-3">
                        <span class="text-lg">🎮</span>
                        <div>
                            <p class="font-medium text-gray-900">${gameType === 'word_match' ? '단어 매칭' : '게임'}</p>
            ...
            <div class="space-y-6">
                ...
                <div class="grid grid-cols-3 gap-4">
                    <div class="bg-purple-50 p-4 rounded-lg text-center">
                        ...
                        <div class="text-lg font-bold text-purple-600">${stats.gameStats.totalGames}</div>
                        <div class="text-sm text-gray-600">총 게임 수</div>
                    </div>
                    <div class="bg-blue-50 p-4 rounded-lg text-center">
                        <div class="text-2xl mb-1">⭐</div>
                        <div class="text-lg font-bold text-blue-600">${Math.round(stats.gameStats.averageScore)}</div>
                        <div class="text-sm text-gray-600">평균 점수</div>
                    </div>
                    <div class="bg-green-50 p-4 rounded-lg text-center">
                        <div class="text-2xl mb-1">🏆</div>
                        <div class="text-lg font-bold text-green-600">${stats.gameStats.bestScore}</div>
                ...
                <div>
                    ...
                    <div class="space-y-2 max-h-64 overflow-y-auto">
                        ${gamesList.length > 0 ? gamesList : '<p class="text-gray-500 text-center py-4">게임 기록이 없습니다.</p>'}
                    </div>
                </div>
                ...
                <div class="bg-gray-50 p-4 rounded-lg">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <p class="font-bold text-purple-600">${Math.floor(stats.gameStats.totalTimePlayed / 60)}분 ${stats.gameStats.totalTimePlayed % 60}초</p>
                        </div>
                        <div>
                            <p class="text-sm text-gray-600">게임당 평균 시간</p>
                            <p class="font-bold text-blue-600">${stats.gameStats.totalGames > 0 ? Math.round(stats.gameStats.totalTimePlayed / stats.gameStats.totalGames) : 0}초</p>
                        </div>
                        <div>
                            <p class="text-sm text-gray-600">점수 향상도</p>
                            <p class="font-bold text-green-600">${stats.gameStats.bestScore > stats.gameStats.averageScore ? '+' + Math.round(stats.gameStats.bestScore - stats.gameStats.averageScore) : '0'}점</p>
                        </div>
                        <div>
                            <p class="text-sm text-gray-600">실력 평가</p>
                            <p class="font-bold text-orange-600">
                                ${stats.gameStats.averageScore >= 300 ? '고수' : 
                                  stats.gameStats.averageScore >= 200 ? '중급자' : 
                                  stats.gameStats.averageScore >= 100 ? '초보자' : '입문자'}
                            </p>
                        </div>
                    </div>
                </div>
...
async function saveLearningActivity(conceptId, targetLanguage, accuracy, studyTime, conceptData = {}) {
    if (!currentUser) return;
    ...
    try {
        ...
        const docRef = await addDoc(collection(db, 'learning_records'), learningRecord);
        console.log('✅ Learning activity saved:', docRef.id);

        // 개념 스냅샷을 user_records에도 저장
        if (conceptData.word || conceptData.term) {
            await saveConceptSnapshotToUserRecords(conceptId, conceptData, targetLanguage);
        }
...
async function saveConceptSnapshotToUserRecords(conceptId, conceptData, targetLanguage) {
    ...
    try {
        ...
        if (userDoc.exists()) {
            ...
            console.log(`✅ 개념 스냅샷 user_records 업데이트: ${conceptId} -> ${conceptSnapshot.word}`);
        } else {
            // 새 문서 생성
            const initialData = {
                concept_snapshots: {
            ...
            console.log(`✅ 개념 스냅샷 user_records 생성: ${conceptId} -> ${conceptSnapshot.word}`);
        }
...
async function saveGameActivity(targetLanguage, score, timeTaken, gameType = 'default', gameData = {}) {
    if (!currentUser) return;
    ...
    try {
        ...
        const gameRecord = {
            ...
            concept_id: gameData.concepts ? gameData.concepts.map(c => c.id).filter(id => id) : [],
            timestamp: new Date(),
            type: 'game'
        };
        ...
        if (gameData.concepts && Array.isArray(gameData.concepts)) {
            for (const concept of gameData.concepts) {
                if (concept.id && concept.word) {
                    await saveConceptSnapshotToUserRecords(concept.id, concept, targetLanguage);
                }
            }
        }
...
async function saveQuizActivity(targetLanguage, score, correctAnswers, totalQuestions, quizData = {}) {
    if (!currentUser) return;
    ...
    try {
        ...
        const quizRecord = {
            ...
            concept_id: quizData.concepts ? quizData.concepts.map(c => c.id).filter(id => id) : [],
            timestamp: new Date(),
            type: 'quiz'
        };
...
async function updateTargetLanguageStats(targetLanguage) {
    ...
    try {
        ...
        if (!docSnap.exists()) {
            ...
            console.log(`✅ Updated ${targetLanguage} complete stats in user_records`);
        }
    ...
    try {
        ...
        await saveGameActivity('english', 320, 165, 'word_match', { 
            ...
            concepts: [
                ...
                { id: 'concept_study_001', word: 'study', domain: '학문', category: '동사' }
            ]
        });
...
window.checkActivityRecords = async function() {
    ...
    const groupedData = {
        english: {
            learning: allLearningRecords.filter(r => r.targetLanguage === 'english'),
            games: allGameRecords.filter(r => r.targetLanguage === 'english'),
            quizzes: allQuizRecords.filter(r => r.targetLanguage === 'english')
        },
        japanese: {
            learning: allLearningRecords.filter(r => r.targetLanguage === 'japanese'),
...
function createConceptSnapshotFromActivity(conceptId, conceptData) {
    return {
        conceptId,
        displayName: conceptData.word || conceptData.term || conceptId.substring(0, 12),
        ...
        targetFlag: getLanguageFlag(conceptData.target_language || selectedTargetLanguage),
        domain: conceptData.domain || '일반',
        category: conceptData.category || '기타',
...
async function fetchAndCreateConceptSnapshot(conceptId) {
    try {
        ...
        if (conceptDoc.exists()) {
            ...
            await saveConceptSnapshotToUserRecords(conceptId, {
                word: targetWord,
    ...
    try {
        ...
        if (userDoc.exists()) {
            ...
            if (conceptSnapshots && conceptSnapshots[conceptId]) {
                ...
                const snapshot = {
                    conceptId,
                    displayName: `${conceptData.source_word || '알 수 없음'} → ${conceptData.word || conceptData.target_word || conceptId.substring(0, 12)}`,
                    sourceLanguage: conceptData.source_language || 'korean',
                    targetLanguage: conceptData.target_language || selectedTargetLanguage,
                    sourceFlag: getLanguageFlag(conceptData.source_language || 'korean'),
                    targetFlag: getLanguageFlag(conceptData.target_language || selectedTargetLanguage),
...
function createDefaultSnapshot(conceptId) {
    return {
        conceptId,
        displayName: conceptId.length > 12 ? conceptId.substring(0, 12) + '...' : conceptId,
...
function calculateActivityAccuracy(record) {
    switch(record.activity_type || inferActivityType(record)) {
        case 'learning':
            return record.accuracy || record.success_rate || record.session_quality || 40;
        case 'game':
...
function extractConceptName(conceptData, conceptId) {
    // 다양한 필드에서 개념명 추출 시도
    const name = conceptData.conceptData?.word || 
                 conceptData.word || 
                 conceptData.concept || 
                 conceptData.term ||
...
async function createAndSaveConceptSnapshot(conceptId, activityData) {
    try {
        ...
        console.log(`🔄 새 스냅샷 생성: ${conceptId} -> ${conceptSnapshot.displayName}`);
        return conceptSnapshot;
